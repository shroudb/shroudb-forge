...
- [ ] **DEBT-8** — `ca_rotate` with Keep configured must clear Store's `key_material` (same contract as DEBT-2 for `ca_create`). Rotation currently writes plaintext hex to both Store and Keep, defeating the defense-in-depth fix from DEBT-2. *No debt test yet; add one before fixing.*
- [ ] **DEBT-9** — `ForgeEngine::new` must reject `Capability::DisabledWithJustification` for Keep when `ForgeConfig::require_keep` is `true`. DEBT-6 landed the flag field-only; server-side enforcement needs an `engine.require_keep` field in `shroudb-forge-server`'s `EngineConfig` plus plumbing through `run_server`. *No debt test yet; add one before fixing.*
