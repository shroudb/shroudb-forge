...
async fn handle_connection<S: Store>(
    ...
    loop {
        ...
        let cmd = match parse_command(&arg_refs) {
            Ok(cmd) => cmd,
            Err(e) => {
                let resp_frame = response_to_frame(&ForgeResponse::error(e));
        ...
        if let ForgeCommand::Auth { ref token } = cmd {
            if let Some(validator) = token_validator {
                match validator.validate(token) {
                    Ok(tok) => {
                        auth_context = Some(tok.into_context());
                        let resp = response_to_frame(&ForgeResponse::ok_simple());
                        write_frame(&mut writer, &resp).await?;
                    }
                    Err(e) => {
                        let resp =
                            response_to_frame(&ForgeResponse::error(format!("auth failed: {e}")));
                        write_frame(&mut writer, &resp).await?;
                    }
                }
            } else {
        ...
        {
            let resp = response_to_frame(&ForgeResponse::error(
...
fn response_to_frame(response: &ForgeResponse) -> Resp3Frame {
    match response {
